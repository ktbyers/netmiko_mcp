...
Bind the MCP server to loopback only (`http_host: "127.0.0.1"`), then let Caddy accept external HTTPS and forward to it:
...
Caddy generates a local CA and issues a certificate automatically. The CA root is stored at:
```
...
Caddy obtains and renews a public certificate automatically. Port 80 and 443 must be reachable from the internet.
...
The MCP endpoint is then reachable at `https://your-mcp-server.example.com/mcp`. The netmiko-mcp bearer auth is still enforced through the proxy.
